    ...
    if (!isguestuser() && isloggedin()) {
        $courseidsql .= "
                SELECT DISTINCT e.courseid
                 ...
                 WHERE c.lang = '$currentlang' AND ue.status = :active AND e.status = :enabled AND ue.timestart < :now1
        ...
        if (is_siteadmin()) {
            ...
            $courseidsql .= "
                    SELECT DISTINCT e.courseid
											FROM {enrol} e
											JOIN {course} c ON e.courseid = c.id
                     WHERE c.lang = '$currentlang' AND e.enrol = 'guest' AND e.password = :emptypass AND e.status = :enabled2";
            $params['emptypass'] = '';
            $params['enabled2'] = ENROL_INSTANCE_ENABLED;
