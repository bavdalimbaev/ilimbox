...
if ($_SESSION['lang']) {
	if ($_SESSION['lang'] == $SESSION->lang) { 
		$categoryid = optional_param('categoryid', null, PARAM_INT);
		if ($_SESSION['catId'] == $categoryid) 
			$categoryid	= $_SESSION['catId'] = ($_SESSION['lang'] == 'ru') ? 13 : 10;
	}	else {
			$_SESSION['lang'] = ($SESSION->lang == null) ? current_language() : $SESSION->lang;
			$categoryid	= $_SESSION['catId'] = ($_SESSION['lang'] == 'ru') ? 13 : 10;
	}
} else {
	$_SESSION['lang'] = ($SESSION->lang == null) ? current_language() : $SESSION->lang;
		...
		case 'bulkaction':
			...
			if ($bulkmovecourses) {
				...
				try {
					...
					$redirectback = \core_course\management\helper::move_courses_into_category(
						$moveto,
						$courseids
					);
					if ($redirectback) {
				...
				if (!in_array($sortcategoriesby, array(
