...
class core_course_category implements renderable, cacheable_object, IteratorAggregate {
    ...
    public static function get_all($options = []) {
        ...
        $catsqlwhere = "WHERE cc.lang = '$currentlanguage' AND ctx.contextlevel = :contextlevel";
    ...
    public static function create($data, $editoroptions = null) {
        ...
        if ($editoroptions) {
            ...
            $updatedata->lang = $newcategory->lang;
    ...
    public function update($data, $editoroptions = null) {
        ...
        if ($data->lang !== $this->lang) {
            if (core_text::strlen($data->lang) > 100) {
                throw new moodle_exception('categorylang');
            }
            $newcategory->lang = $data->lang;
        }
    ...
    protected static function get_records($whereclause, $params) {
        ...
        $sql = "SELECT cc.". join(',cc.', $fields). ", $ctxselect
                ...
                WHERE cc.lang = '$currentlang' AND ". $whereclause." ORDER BY cc.sortorder";
    ...
    protected static function get_course_records($whereclause, $params, $options, $checkvisibility = false) {
        ...
        $sql = "SELECT ". join(',', $fields). ", $ctxselect
                ...
                WHERE c.lang = '$currentlang' AND ". $whereclause." ORDER BY c.sortorder";
        ...
        if ($checkvisibility) {
            ...
            foreach ($list as $course) {
				...
				// echo '<pre>'; var_dump($mycourses); echo '</pre>';
    ...
    public function has_courses() {
        global $DB;
        $currentlang = current_language();
        return $DB->record_exists_sql("SELECT 1 FROM {course} WHERE lang = '$currentlang' AND category = ?",
                array($this->id));
    }
    ...
    public function resort_courses($field, $cleanup = true) {
        ...
        $sql = "SELECT c.id, c.sortorder, c.{$field}, $ctxfields
             ...
             LEFT JOIN {context} ctx ON ctx.instanceid = c.id
                 WHERE c.lang = '$currentlang' AND
                        ctx.contextlevel = :ctxlevel AND
